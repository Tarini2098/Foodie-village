...
export class AddItemComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    if (formData.get('file') == null || formData.get('file') == undefined) {
      console.log(formData.get('file'));
      this.url = "http://localhost:1200/addNewItem";
    }
    else {
      this.url = "http://localhost:1200/addNewItemUrl";
    }
  ...
  checkAvailability() {
    this.http.post<boolean>("http://localhost:1200/checkItemId", this.newFoodItems.id).subscribe(
      res => {
        this.present = res;
      }, err => {
